...
use std::{ str, time::{ Duration, Instant }, env, default::Default, sync::RwLock, error::Error, sync::Arc, mem::drop, convert::TryInto, collections::HashMap };
                ...
                Control::Tick => {
                    ...
                    if count < config.targetpeers.into() {
                        ...
                        if ports.is_empty() {
                            ...
                            let tx = tx.clone();
                            tokio::spawn(async move {
                                tcp::connect_node(config, tx, ports, false).await;
                            });
                        }
                    }
